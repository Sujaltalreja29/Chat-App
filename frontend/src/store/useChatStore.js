...
export const useChatStore = create((set, get) => ({
  ...
  subscribeToMessages: () => {
    ...
    // 🔧 FIX: Add null check for authUser
    if (!authUser) {
    ...
    const setupEventListeners = () => {
      ...
      socket.on("newMessage", (newMessage) => {
        // 🔧 FIX: Add validation for newMessage
      ...
      socket.on("newGroupMessage", (newMessage) => {
        // 🔧 FIX: Add validation for newMessage
        if (!newMessage || !newMessage.groupId) {
          console.error("Invalid group message received:", newMessage);
          return;
        }
